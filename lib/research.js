  ...
  const yahooPromise = (async () => {
    try {
      ...
      const [quote, historicalData] = await Promise.all([
        yahooFinance.quoteSummary(identity.ticker, {
          modules: ['summaryProfile', 'summaryDetail', 'defaultKeyStatistics', 'financialData']
        }),
        yahooFinance.chart(identity.ticker, {
          period1: startDate,
          period2: endDate,
          interval: "1d"
        }).catch(err => {
          console.warn(`Historical data failed for ${identity.ticker}`, err);
          return { quotes: [] };
        })
      ]);
      ...
      if (!detail.marketCap && !fin.totalRevenue) {
        ...
        financials = {
          historical: (historicalData.quotes || []).map(h => ({ date: h.date, close: h.close })),

...
export async function gatherResearchData(identity) {
  ...
  const fmpPromise = (async () => {
    ...
    try {
      ...
      if (!profile.marketCap && !latestIncome.revenue) {
        errors.push("FMP returned empty data (likely due to free-tier restrictions on international stocks).");
        financials = null;
      } else {
        financials = {
          ...
          netMargin: netMargin,
          returnOnEquity: metrics.returnOnEquityTTM !== undefined ? metrics.returnOnEquityTTM * 100 : null,
